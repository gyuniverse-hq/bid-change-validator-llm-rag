...
def _extract_evidence(rule, chunk, std_clause=None):
    ...
    if not client.available:
        return None
    topic = f"{rule['name']} — {rule['std_desc']}"
...
def find_via_embedding(rule, std_clause, chunks, exclude_ids):
    ...
    query = f"{rule['name']} — {rule['std_desc']}. {std_clause['text'][:300]}"
    try:
    ...
    for chunk in top:
        evidence = _extract_evidence(rule, chunk, std_clause)
        if evidence is None:
            continue
        ...
        if rule["direction"] == "text":
            ...
            if ownership == "발주기관단독":
                return ("확인 필요",
                        f"표준({rule['std_desc']})과 다른 단독 귀속 문언(AI 보강 탐지)",
        ...
        if not value_raw:
            continue
        values = [v for v in extract_values(value_raw) if v["unit"] == rule["unit"]]
        ...
        if minf is not None and val["value"] < minf:
            continue
        if val["value"] > rule["std_value"] + 1e-4:
            return ("확인 필요", f"RFP 값이 표준({rule['std_desc']})을 초과(AI 보강 탐지)",
                    val, evidence["quote"], chunk)
        return ("적합", f"표준({rule['std_desc']}) 이내(AI 보강 탐지)",
                val, evidence["quote"], chunk)
