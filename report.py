...
PENALTY_RATE_EXCLUSION = (
    "지체상금율 — 시행규칙 제75조 원문 미확보로 판정 제외 "
    "(용역계약일반조건 제55조는 시행규칙 제75조로 위임만 하고 있음)"
)
...
def _narrative_summary(report_text):
    ...
    try:
        out = client.chat(
            "아래는 코드가 확정한 입찰공고 분석 리포트다. 판정을 바꾸거나 새 판정을 추가하지 말고, "
            "담당자가 읽기 좋은 3~5문장 요약만 작성하라. 모든 수치는 리포트에 있는 그대로 사용하라.",
            report_text)
